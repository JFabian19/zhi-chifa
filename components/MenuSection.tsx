  ...
  return (
    <section id={id} className="pt-8 pb-4 scroll-mt-20">
      <div className="flex flex-col items-center mb-6">
        <h2 className="text-2xl font-oriental text-chifa-wine text-center">
          {titleMain}
        </h2>
        {titleSub && (
          <span className="text-chifa-gold font-body uppercase text-xs tracking-widest mt-1">
            {titleSub}
          </span>
        )}
        <div className="w-16 h-1 bg-chifa-red mt-2 rounded-full opacity-80"></div>

        <div className="w-full aspect-[3/2] bg-gray-200 mt-4 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-400">
          <span className="text-gray-500 font-body font-medium">aca va imagen</span>
        </div>

        {category.descripcion && (
          <p className="text-gray-500 text-sm mt-2 text-center max-w-xs font-body italic">
            {category.descripcion}
          </p>
        )}

        {category.precio_general && (
          <div className="mt-3 bg-chifa-gold/10 text-chifa-wine px-4 py-1 rounded-full font-bold font-body text-sm border border-chifa-gold/30">
